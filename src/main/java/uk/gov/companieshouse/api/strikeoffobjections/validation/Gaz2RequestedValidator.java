...
public class Gaz2RequestedValidator {
    ...
    public void validate(String companyNumber, Long actionCode, String logContext)
        ...
        if (gaz1ActionCode == actionCode) {
            ...
            apiLogger.debugContext(
                    logContext, "Company action code is GAZ1, checking for requested GAZ2");
            ...
            if (requestedGaz2Result != null) {
                apiLogger.infoContext(
                        logContext, "Company has a requested GAZ2 transaction ,failing validation", logMap);
                throw new ValidationException(EligibilityStatus.INELIGIBLE_GAZ2_REQUESTED);
            }

            apiLogger.debugContext(
                    logContext, "Company does not have requested GAZ2 transaction, passing validation");
        }
    }
}
